# Shared structure; small per-host swaps for wording, the brightness spawn
# style, the extra kbd-backlight keys (laptop only), whether workspaces 4/5
# are named ("games"/"mail" on desktop) or just numbered (laptop), and the
# keyboard-layout-switch bind (desktop drives it from a physical key via a
# script, laptop just uses a bind).
{
  hostname,
  lib,
  ...
  ...
  ws4 = if isLaptop then "4" else "\"games\"";
  ws5 = if isLaptop then "5" else "\"mail\"";

  kbdBacklightBinds = lib.optionalString isLaptop ''

    XF86KbdBrightnessUp allow-when-locked=true { spawn-sh "brightnessctl --device='rgb:kbd_backlight' set +20%"; }
    XF86KbdBrightnessDown allow-when-locked=true { spawn-sh "brightnessctl --device='rgb:kbd_backlight' set 20%-"; }'';

  kbdLayoutSwitchBind =
    if isLaptop
    then ''    Mod+Shift+Ctrl+O { switch-layout "prev"; }''
    else ''    XF86Tools       { spawn-sh "~/.config/niri/scripts/kbd-layout.sh --next"; } // key is F13 in keychron launcher assigned to the key above num-lock
    // Mod+Shift+Ctrl+O { switch-layout "prev"; }'';
in ''
binds {
...
${kbdBacklightBinds}
    ...
    Mod+2 { focus-workspace "discord"; }
    Mod+3 { focus-workspace "terminal"; }
    Mod+4 { focus-workspace ${ws4}; }
    Mod+5 { focus-workspace ${ws5}; }
    Mod+6 { focus-workspace 6; }
    Mod+7 { focus-workspace 7; }
    Mod+8 { focus-workspace 8; }
    Mod+9 { focus-workspace 9; }
    Mod+Shift+1 { move-column-to-workspace "browser"; }
    Mod+Shift+2 { move-column-to-workspace "discord"; }
    Mod+Shift+3 { move-column-to-workspace "terminal"; }
    Mod+Shift+4 { move-column-to-workspace ${ws4}; }
    Mod+Shift+5 { move-column-to-workspace ${ws5}; }
